...
//check this line of code incase there is an error later in the future
    ...
    return(
        <Dialog open ={true} onClose = {handleClose} fullWidth>
            ...
            <DialogContent>
                <Box mb ={3}>
                    <TextField error={!!errors.name} helperText ={errors.name} value={form.name}
                    name = "name" label = "Name" onChange = {handleChange}  fullWdith variant="filled"/>
                </Box>
                <TextField error={!!errors.longURL} helperText ={errors.longURL} value={form.longURL}
                    name = "longURL" label = "Long URL" onChange = {handleChange}  fullWdith variant="filled"/>
            </DialogContent>

            <DialogActions>
                <Box mr={2} my={1}>
                    <Button onClick ={handleSubmit} color= "primary" variant="filled" disableElevetaion disabled ={loading}>
                        {loading ? (<CircularProgress color ="inherit" size = "23"/>):("Create Short URL")}
                    </Button>
                </Box>
            </DialogActions>
